...
import { api, loginUser } from "../http";
...
const Login = () => {
  ...
  const click = async (e) => {
    ...
    try {
      ...
      await api.post(`/api/users/online`, { userId: userData._id, onlineStatus: true });
      localStorage.setItem("UserData", data);
      router.push("/");
    } catch (err) {
      toast({
        title: "Login Error",
        description: "Invalid Credentials. Please enter valid credentials",
        status: "error",
        duration: 9000,
        isClosable: true,
      });
      ...
      <div className={styles.container}>
        <div className={styles.logincontainer}>
          <div className={styles.left}>
            <div>
              <svg className={styles.logo} xmlns="http://www.w3.org/2000/svg" aria-label="Facebook logo" role="img" viewBox="0 0 120 24">
          ...
          <div className={styles.right}>
            <form>
              ...
              <button className={styles.login} onClick={click}>
                Log in
              </button>
              <a>Forgot password?</a>
              <hr />
              <button onClick={registerFormHandler} className={styles.register}>
                Create New Account
              </button>
            </form>
          </div>
          {registerForm && <RegisterForm handler={registerFormHandler} />}
        </div>
      </div>
    </>
  );
